...
public class ConfigurationManager {
	 ...
	 * 就由每个类内部的static {}构成的静态代码块决定，自己可以在类中开发静态代码块
	 * 类第一次使用的时候，就会加载，加载的时候，就会初始化类，初始化类的时候就会执行类的静态代码块
	 * 
	 * 因此，对于的配置管理组件，就在静态代码块中，编写读取配置文件的代码
	...
	static {
		try {
			...
			InputStream in = ConfigurationManager.class
					.getClassLoader().getResourceAsStream("prop.properties"); 
	 ...
	 * 类，和执行静态代码块了，所以也印证了上面所说的，类只会加载一次，配置文件也仅仅会加载一次
	 * 
	 * @param key 
	 * @return value
	 */
